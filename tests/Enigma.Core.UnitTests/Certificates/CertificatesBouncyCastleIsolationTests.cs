...
/// <summary>
/// Principle 1, scoped to <c>Enigma.Core.Certificates</c> (plus the shared <see cref="RsaSignatureAlgorithm"/>
/// enum): certificates, CSRs, CRLs and keys cross the API as PEM strings, with extension controls expressed by
/// the BouncyCastle-free <see cref="X509KeyUsage"/> / <see cref="X509CertificateOptions"/> types, so this guard
/// walks every exported type in the namespace and fails if any <c>Org.BouncyCastle.*</c> type appears on a base
/// type, implemented interface, exposed method return/parameter, constructor parameter, or exposed field.
/// </summary>
public class CertificatesBouncyCastleIsolationTests
{
    private const string ForbiddenNamespaceRoot = "Org.BouncyCastle";
    private const string CertificatesNamespace = "Enigma.Core.Certificates";

    [Fact]
    public void NoCertificatesType_ExposesBouncyCastleType()
    {
        var types = typeof(IX509CertificateService).Assembly
            .GetExportedTypes()
            .Where(t => t.Namespace == CertificatesNamespace)
            .Append(typeof(RsaSignatureAlgorithm))
            .ToList();

        Assert.Contains(typeof(X509CertificateService), types);
        Assert.Contains(typeof(X509CertificateServiceFactory), types);
        Assert.Contains(typeof(X509CertificateOptions), types);
        Assert.Contains(typeof(X509KeyUsage), types);
        Assert.Contains(typeof(CertificateInfo), types);
        ...
        Assert.True(offenders.Count == 0,
            "BouncyCastle types leaked onto the certificates public surface:" + Environment.NewLine +
