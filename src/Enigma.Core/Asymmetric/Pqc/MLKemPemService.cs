...
/// <summary>
/// Default <see cref="IMLKemPemService"/> implementation, backed by BouncyCastle's PKCS#8 / SubjectPublicKeyInfo
/// encoders and the library's shared PEM envelope.
/// </summary>
/// <remarks>
/// Construct directly with <c>new</c> or through <see cref="MLKemPemServiceFactory"/>; the service is stateless, so
...
/// </remarks>
public sealed class MLKemPemService : IMLKemPemService
{
    /// <inheritdoc />
        ...
        MLPrivateKeyPemFormat format = MLPrivateKeyPemFormat.Seed)
    {
        ...
        var privateKey = ((MLKemPrivateKeyParameters)pair.Private).WithPreferredFormat(preferredFormat);
    ...
    /// <inheritdoc />
    public string ToPublicKeyPem(byte[] publicKey, MLKemParameterSet parameterSet)
    {
        ...
        MLKemPublicKeyParameters key;
        try
        {
            key = MLKemPublicKeyParameters.FromEncoding(parameters, publicKey);
        }
        catch (Exception ex) when (IsKeyInputFailure(ex))
        {
            throw new ArgumentException(
                $"The bytes are not a valid ML-KEM public key for {parameterSet}.", nameof(publicKey), ex);
        }
    ...
    /// <inheritdoc />
    public string ToPrivateKeyPem(byte[] privateKey, MLKemParameterSet parameterSet, char[]? password = null)
    {
        if (privateKey is null) throw new ArgumentNullException(nameof(privateKey));

        var parameters = MLParameterSets.ToBcParameters(parameterSet);

        MLKemPrivateKeyParameters key;
        try
        {
            ...
            key = MLKemPrivateKeyParameters.FromEncoding(parameters, privateKey)
                .WithPreferredFormat(MLKemPrivateKeyParameters.Format.EncodingOnly);
        }
        catch (Exception ex) when (IsKeyInputFailure(ex))
        {
            throw new ArgumentException(
                $"The bytes are not a valid ML-KEM private key for {parameterSet}.", nameof(privateKey), ex);
        }
    ...
    /// <inheritdoc />
    public (byte[] publicKey, MLKemParameterSet parameterSet) FromPublicKeyPem(string pem)
    {
        if (pem is null) throw new ArgumentNullException(nameof(pem));

        if (PemEnvelope.ReadPublicKey(pem, nameof(pem)) is not MLKemPublicKeyParameters key)
            throw new ArgumentException("The PEM does not contain an ML-KEM public key.", nameof(pem));

        return (key.GetEncoded(), MLParameterSets.FromBcParameters(key.Parameters, nameof(pem)));
    }

    /// <inheritdoc />
    public (byte[] privateKey, MLKemParameterSet parameterSet) FromPrivateKeyPem(string pem, char[]? password = null)
    {
        if (pem is null) throw new ArgumentNullException(nameof(pem));

        if (PemEnvelope.ReadPrivateKey(pem, password, nameof(pem)) is not MLKemPrivateKeyParameters key)
            throw new ArgumentException("The PEM does not contain an ML-KEM private key.", nameof(pem));

        // GetEncoded returns the expanded FIPS 203 decapsulation key whatever format the PEM stored: a seed-only
        // key is expanded here, so the caller always receives bytes that IMLKemService.Decapsulate accepts.
        return (key.GetEncoded(), MLParameterSets.FromBcParameters(key.Parameters, nameof(pem)));
    }

    // Maps the library's PEM format enum onto BouncyCastle's encoding preference. The ML-DSA and ML-KEM Format
    // enums are two distinct nested types, so this switch cannot be shared with MLDsaPemService.
    private static MLKemPrivateKeyParameters.Format ToBcFormat(MLPrivateKeyPemFormat format) => format switch
    {
        MLPrivateKeyPemFormat.Seed => MLKemPrivateKeyParameters.Format.SeedOnly,
        MLPrivateKeyPemFormat.ExpandedKey => MLKemPrivateKeyParameters.Format.EncodingOnly,
        MLPrivateKeyPemFormat.SeedAndExpandedKey => MLKemPrivateKeyParameters.Format.SeedAndEncoding,
